...
export default function CodingArea() {
  ...
  const handleSubmitCode = async () => {
    ...
    const currentQ = questions[curIdx]?.questions_id;
    const code = codeByQ[currentQ.id];
    const ext = LANG_OPTIONS.find(l => l.value === lang)?.ext || 'java';
  ...
  const currentExpectedOutputs = (() => {
    ...
    if (!currentQ) return [];
    try {
      const testcasesObj = JSON.parse(currentQ.testcases || "{}");
      const keys = Object.keys(testcasesObj).slice(0, 2);
      return keys.map(k => testcasesObj[k].output || "");
    } catch {
      return [];
    }
  })();
