...
function getInitialTimerValue(initialMinutes) {
  if (initialMinutes === 'unlimited') return 0;
...
export default function CodingArea() {
  ...
  const fetchQuestionsAndTimer = useCallback(async () => {
    try {
      ...
      setQuestions(data);
      const initialStatus = {};
      data.forEach((_, idx) => (initialStatus[idx] = 'unseen'));
      setStatusMap(initialStatus);
      setNotViewedCount(data.length);
      setSavedInServerCount(0);
      const timerFromServer = 'unlimited';
      setTotalTimeMinutes(timerFromServer);
      setTimeLeft(getInitialTimerValue(timerFromServer));
      setTimerActive(true);
      if (data.length > 0) {
        setCurQuestionIdx(0);
        setQuestionContent(data[0]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (roomCode) fetchQuestionsAndTimer();
  ...
  const handleRunCode = async () => {
    ...
    setRunResult({ testcases: [{ id: 1, passed: true }, { id: 2, passed: false }] });
    setLoading(false);
  };
  ...
  const handleSubmitCode = async () => {
    ...
    setSubmitResult({ testcases: [{ id: 1, passed: true }, { id: 2, passed: true }] });
  ...
  const handleLangChangeAlt = (e) => {
    ...
    setLang(newLang);
    setCodeByQ((prev) => ({ ...prev, [questions[curIdx]?.questions_id?.id]: defaultCodeByLang[newLang] }));
    setCode(initialCodeSamples[newLang]);
  };

  const handleCodeChange = (val) => {
    setCodeByQ((prev) => ({ ...prev, [questions[curIdx]?.questions_id?.id]: val }));
  };

  const displayCode =
    (questions[curIdx]?.questions_id?.id && codeByQ[questions[curIdx]?.questions_id?.id]) || code;

  return (
    <>
      <TopBar username={user?.username || authUser?.username || 'Guest'} roomCode={roomCode} onSubmitTest={handleSubmitTest} />
      {showSubmitModal && (
      ...
      <main className={styles.mainLayout}>
        ...
        <CodeEditorSection
          lang={lang}
          setLang={(val) => { setLang(val); handleLangChangeAlt({ target: { value: val } }); }}
          code={displayCode}
          setCode={handleCodeChange}
          onClear={() => setCode(initialCodeSamples[lang])}
          onRun={handleRunCode}
